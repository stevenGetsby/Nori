...
| Holly case tests fail when the fixture is absent | Intaker/AccountPlanner tests reference `cases/Holly/showcase/brief.md` and curated showcase images. | Keep the Holly case fixture in `cases/Holly` or mark those tests fixture-dependent if it is intentionally removed. |
...
| README drifts back to historical archive entrypoints | Older docs predate the canonical wiki and may reference missing files. | Keep README as a short navigation page that points to `wiki/`; put durable project truth in wiki. |
...
| Duplicate concept spread across archived docs and wiki | Historical docs predate wiki. | Treat `wiki/archive/` as historical only; put durable current truth in wiki. |
