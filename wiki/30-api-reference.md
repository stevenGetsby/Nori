...
| `llms.errors.*` | Import path | Exception classes | Compatibility import path for `LLMClientConfigError`, `ChatJSONError`, `ChatResultError`, `ChatCapabilityError`, `ImageCapabilityError`, and `ImageResultError`; canonical ownership is `nori.core.contracts`, and package/legacy module imports preserve the same class identity. |
...
| `llms.structured_models.{StructuredCallResult,IntentLLMResult,TargetSelectionResult}` | Structured helper fields | Dataclass contracts | Compatibility import path for non-throwing structured-helper result models; canonical ownership is `nori.core.contracts`, and package/legacy module imports preserve the same class identities. |
...
| `nori.core.contracts.{ProviderConfig,ModelConfig,ResolvedModel}` | Config model fields | Dataclass contracts | Canonical runtime-config dataclasses shared by `nori_config` and `llms`; `nori.core`, `nori.config_models`, and legacy imports from `nori.nori_config` preserve the same class identities. |
