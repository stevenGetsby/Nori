...
| `llms.errors.*` | Import path | Exception classes | Canonical source for `LLMClientConfigError`, `ChatJSONError`, `ChatResultError`, `ChatCapabilityError`, `ImageCapabilityError`, and `ImageResultError`; package and legacy module imports preserve the same class identity. |
...
| `llms.structured_models.{StructuredCallResult,IntentLLMResult,TargetSelectionResult}` | Structured helper fields | Dataclass contracts | Canonical non-throwing structured-helper result models; package and legacy module imports preserve the same class identities. |
...
| `nori.config_models.{ProviderConfig,ModelConfig,ResolvedModel}` | Config model fields | Dataclass contracts | Canonical runtime-config dataclasses shared by `nori_config` and `llms`; legacy imports from `nori.nori_config` preserve the same class identities. |
...
| `nori.content_generation.content_producer.inputs.*` | Internal production input helpers for dict asset restoration, task/brief text-context fallback, intent/context construction, and draft skill selection. |
| `nori.content_generation.content_producer.builder.normalize_assets(assets, task, brief) -> list[UserAsset]` | Legacy-compatible proxy to `content_package_inputs.normalize_assets`. |
| `nori.content_generation.content_producer.builder.package_from_outputs(task, draft, cover, ...) -> ContentPackage` | Internal production helper for mapping `NoteDraft` / `CoverResult` outputs into package prompts, media paths, material usage, source refs, and production metadata. |
| `nori.content_generation.content_producer.refs.*` | Internal production provenance helpers for stable package IDs, image-path dedupe, material-usage rows, and source refs used by `ContentPackage`. |
...
Business-module model files should support `to_dict()` where the object crosses a stage boundary. Cross-stage contracts such as `UserAsset`, `AssetRecord`, `AssetLibrary`, `ClientBrief`, `OperationPlan`, `KPIPlan`, `ContentTask`, and `ContentCalendar` live in `nori.core.models`; the cross-module project aggregate `AccountOperationProject` lives in `nori.core.project`. `UserInput`, `IntakeResult`, `AccountPlannerInput`, and `AccountPlanResult` live in `nori.user_profiling.models`; `AssetBundle`, `CandidateTitle`, `NoteDraft`, `CoverResult`, and `ContentPackage` live in `nori.content_generation.models`; `XHSNoteSample`, `XHSSeedSkillDraft`, `NoteSkill`, `NoteEvidence`, and `SessionSkillReport` live in `nori.market_analysis.models`. The old `nori.agent_models` compatibility root has been removed; import models from the owning business module. Model `from_dict()` methods use `nori._model_coercion` for shared mapping/list/string/int/bool cleanup.

Canonical business model ownership:

| Module | Models |
| --- | --- |
| `nori.user_profiling.models` | `UserInput`, `IntakeResult`, `AccountPlannerInput`, `AccountPlanResult`, `AccountPositioning`; re-exports `ClientBrief` from `nori.core` |
...
| `nori.context_building.models` | Compatibility re-exports for `AssetRecord`, `AssetLibrary`, `AccountOperationProject`, `OperationPlan`, `KPIPlan`, `ContentCalendar`, and `ContentTask` from `nori.core` |
...
| `nori.content_generation.models` | `AssetBundle`, `CandidateTitle`, `NoteDraft`, `CoverResult`, `ContentPackage`; re-exports `UserAsset` from `nori.core` for generation call-site compatibility |
| `nori.learning_loop.models` | `ComplianceReview`, `MetricsSnapshot`, `StrategyIteration` |

The old account-ops model compatibility root has been removed. Import models from the owning business module listed above.
