...
| Add public capability entrypoint | Done | `nori/capabilities/` exposes `build_capability_snapshot()` and `validate_capability_snapshot()` for upper-layer CLI/API/UI consumers. |
| Introduce LangGraph workflow execution | Done | Added `LangGraphWorkflowRunner`; `WorkflowRunner` compiles `WorkflowSpec` into a LangGraph `StateGraph` and records stage status/artifact refs. |
