...
| Introduce LangGraph workflow execution | Done | Added `LangGraphWorkflowRunner`; `WorkflowRunner` compiles `WorkflowSpec` into a LangGraph `StateGraph`, supports `HumanGateSpec` with default skip mode, and records stage status/artifact refs. |
