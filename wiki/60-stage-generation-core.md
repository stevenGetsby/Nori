...
| `llms.json_calls.*` | Canonical JSON-mode raw-call and retry plumbing. It owns `response_format` fallback classification and copies request params before retrying; `llms.call._chat_json_raw`, `_without_response_format`, `_should_retry_without_response_format`, and `_is_response_format_error` remain compatibility aliases. |
...
| `llms.request_params.merge_chat_kwargs(...)` | Canonical chat gateway merge for model constraints. It copies caller `extra_body` before applying model-level defaults and always normalizes token-limit kwargs so each model sends only its expected parameter, even when no `max_output` default is configured. `llms.call._merge_kwargs` remains a compatibility alias. |
...
| `llms.image_inputs.*` | Canonical reference-image input helpers. `load_image_bytes(...)` normalizes bytes, data-uri, local path, and base64 string inputs while returning empty bytes for invalid/remote values; `sniff_mime(...)` and `bytes_to_data_uri(...)` build provider payloads. `llms.call._load_image_bytes`, `_sniff_mime`, and `_bytes_to_data_uri` remain compatibility aliases. |
| `llms.image_providers.*` | Canonical provider-specific image helpers. OpenAI-compatible edit, relay reference-payload retry, and Google native image generation live here, while `llms.call._image_openai_edit`, `_image_relay_generate_with_references`, `_image_google`, and `_is_retryable_relay_reference_error` remain compatibility aliases. |
| `llms.structured_outputs.*` | Canonical cleanup and parse-error classification for optional structured LLM helpers. `llms.intent_extractor._clean_str` and `llms.target_selector._clean_str` remain compatibility aliases. |
