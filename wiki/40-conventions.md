...
| Model coercion | `from_dict()` methods should use shared helpers from `nori._model_coercion` for mapping/list/string/int/bool cleanup instead of duplicating private helper functions per model file. Do not use plain `bool(value)` for persisted strings such as `"false"`. |
...
| Runtime config models | Keep `ProviderConfig`, `ModelConfig`, and `ResolvedModel` in `nori.config_models`; `nori_config` may re-export them for compatibility but should not own their definitions. |
...
| Structured LLM helper results | Keep `StructuredCallResult`, `IntentLLMResult`, and `TargetSelectionResult` in `llms.structured_models`; intent/target/call modules may re-export them for compatibility but should not own their definitions. |
