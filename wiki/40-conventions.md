...
| Model coercion | `from_dict()` methods should use shared helpers from `nori.core.contracts` for mapping/list/string/int/bool cleanup instead of duplicating private helper functions per model file. Do not use plain `bool(value)` for persisted strings such as `"false"`. `nori._model_coercion` is compatibility-only. |
...
| Runtime config models | Keep `ProviderConfig`, `ModelConfig`, and `ResolvedModel` in `nori.core.contracts`; `nori.core`, `nori.config_models`, and `nori_config` may re-export them for compatibility but should not own their definitions. |
...
| Structured LLM helper results | Keep `StructuredCallResult`, `IntentLLMResult`, and `TargetSelectionResult` in `nori.core.contracts`; `llms.structured_models`, intent, target, and call modules may re-export them for compatibility but should not own their definitions. |
