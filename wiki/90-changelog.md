...
| Standardized stage support modules | Prompt-owning workflow stages keep `prompts.py`; model contracts stay in canonical `models.py` files or `nori.core`, and CoverDirector's prompt boundary moved from singular `prompt.py` to canonical `prompts.py`. |
