...
from nori.agent_models import NoteSkill, UserAsset
from nori.gen_agents import NoteMakerAgent
from nori.gen_agents.note_maker import NoteMakerLLMError

note_maker_module = importlib.import_module("nori.gen_agents.note_maker")
...
def test_note_maker_routes_json_calls_through_llms_chat_json(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", sentinel_chat)
    monkeypatch.setattr(note_maker_module.llms, "chat_json", fake_chat_json)
...
def test_note_maker_single_skill_skips_picker(monkeypatch):
    ...
    log: list[dict] = []
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat(log, [_CURATE_JSON, _COMPOSE_JSON]))
...
def test_note_maker_uses_main_image_as_cover(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], [curate_json, _COMPOSE_JSON]))
...
def test_note_maker_propagates_validation_from_llm(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], [_CURATE_JSON, compose_json]))
...
def test_note_maker_raises_when_compose_missing_title(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], [_CURATE_JSON, bad_compose]))
...
def test_note_maker_raises_when_picker_returns_unknown_id(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], [pick_json]))
...
def test_note_maker_raises_when_llm_chat_throws(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", boom)
...
def test_note_maker_raises_when_llm_returns_non_json(monkeypatch):
    skills = [_planting_skill()]
    assets = [UserAsset(kind="text", text="x")]
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], ["this is not json"]))
...
def test_note_maker_accepts_note_skill_dataclass(monkeypatch):
    ...
    monkeypatch.setattr(note_maker_module.llms, "chat", _stub_chat([], [_CURATE_JSON, _COMPOSE_JSON]))
