...
from nori.agent_models import CandidateTitle, CoverResult, NoteDraft, UserAsset
from nori.gen_agents import CoverDirectorAgent
from nori.gen_agents.cover_director import CoverDirectorError

cd_module = importlib.import_module("nori.gen_agents.cover_director")
...
def test_cover_director_writes_local_png_from_data_uri(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cd_module.llms, "chat", fake_chat)
    monkeypatch.setattr(cd_module.llms, "image", fake_image)
...
def test_cover_director_writes_png_from_http_url(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cd_module.llms, "chat", lambda *a, **k: _PROMPT_JSON)
    monkeypatch.setattr(cd_module.llms, "image", lambda *a, **k: ["https://example.com/cover.png"])
...
def test_cover_director_raises_when_prompt_empty(tmp_path, monkeypatch):
    draft = _draft()
    skill = _planting_skill()
    monkeypatch.setattr(cd_module.llms, "chat", lambda *a, **k: json.dumps({"prompt": ""}))
...
def test_cover_director_raises_when_image_api_empty(tmp_path, monkeypatch):
    ...
    skill = _planting_skill()
    monkeypatch.setattr(cd_module.llms, "chat", lambda *a, **k: _PROMPT_JSON)
    monkeypatch.setattr(cd_module.llms, "image", lambda *a, **k: [])
...
def test_cover_director_raises_when_image_api_throws(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cd_module.llms, "image", boom)
...
def test_cover_director_raises_when_chat_returns_non_json(tmp_path, monkeypatch):
    draft = _draft()
    skill = _planting_skill()
    monkeypatch.setattr(cd_module.llms, "chat", lambda *a, **k: "no json")
...
def test_cover_director_caps_tagged_selection_at_max_references(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cd_module.llms, "chat", fake_chat)
    monkeypatch.setattr(cd_module.llms, "image", lambda *a, **k: [_TINY_PNG_DATA_URI])
