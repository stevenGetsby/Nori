...
def test_xhs_note_analyzer_falls_back_when_llm_fails(monkeypatch, tmp_path):
    ...
    assert data["validation"]["llm_error"]["stage"] == "xhs_note_analyzer"
    assert data["validation"]["llm_error"]["reason"] == "api_error"
    assert data["validation"]["llm_error"]["error_type"] == "RuntimeError"
    assert data["craft"]["title_rules"]
...
def test_collect_for_session_routes_required_json_stages_through_shared_helper(monkeypatch, tmp_path):
    ...
    report = analyzer.collect_for_session(
        {"platform": "xhs", "topic": "封面设计", "data_dir": str(tmp_path)},
        dc=collector,
    )
