...
import nori.workflows as workflows
from nori.core import ArtifactStore, WorkflowBase
from nori.workflows import HumanGateRequired, HumanGateSpec, StageSpec, StageTimeoutError, WorkflowSpec, WorkflowRunner
...
def test_root_project_declares_langchain_and_langgraph_dependencies():
    ...
    assert '"langchain-openai>=0.3.35,<0.4"' in source
    assert '"langgraph>=0.6.10,<0.7"' in source


def test_workflow_runner_owns_langgraph_execution_without_nested_runner_file():
    assert WorkflowRunner.__module__ == "nori.workflows.runner"
    assert not hasattr(workflows, "LangGraphWorkflowRunner")
...
def test_workflow_runner_executes_stage_spec_through_langgraph():
    ...
    assert run.metadata["checkpoint_saver"] == ""


def test_workflow_runner_default_handles_runtime_only_state_without_checkpoint_serialization():
    class RuntimeOnly:
        pass

    runtime = RuntimeOnly()
    spec = WorkflowSpec(
        name="runtime_only_graph",
        stages=[StageSpec("keep_runtime", lambda value: {**value, "ok": True})],
    )

    output, run = WorkflowRunner().run(spec, {"runtime": runtime})
...
def test_workflow_runner_times_out_configured_stage_and_records_failure():
    spec = WorkflowSpec(
        name="timeout_graph",
        stages=[
            StageSpec("slow", lambda value: time.sleep(1) or value, timeout_seconds=0.05),
            StageSpec("never", lambda value: {**value, "done": True}),
        ],
    )

    with pytest.raises(StageTimeoutError, match="slow") as exc_info:
        WorkflowRunner().run(spec, {"ok": True})

    workflow_run = exc_info.value.workflow_run

    assert workflow_run.status == "failed"
    assert [stage.stage_name for stage in workflow_run.stages] == ["slow"]
    assert workflow_run.stages[0].status == "failed"
    assert workflow_run.stages[0].metadata["timeout_seconds"] == 0.05
    assert workflow_run.stages[0].metadata["timeout_enforcement"] == "signal"
    assert workflow_run.stages[0].metadata["timeout_cancellation"] == "interrupts_current_call"
    assert "StageTimeoutError: stage 'slow' exceeded 0.05s timeout" in workflow_run.stages[0].error


def test_workflow_runner_times_out_configured_stage_from_worker_thread():
    spec = WorkflowSpec(
        name="threaded_timeout_graph",
        stages=[StageSpec("slow", lambda value: time.sleep(1) or value, timeout_seconds=0.05)],
    )
    errors = []
    ...
    assert len(errors) == 1
    assert isinstance(errors[0], StageTimeoutError)
    workflow_run = errors[0].workflow_run
    assert workflow_run.stages[0].stage_name == "slow"
    assert workflow_run.stages[0].status == "failed"
    assert workflow_run.stages[0].metadata["timeout_seconds"] == 0.05
    assert workflow_run.stages[0].metadata["timeout_enforcement"] == "worker_thread"
    assert workflow_run.stages[0].metadata["timeout_cancellation"] == "request_returns_worker_may_continue"
