...
from nori.gen_agents import IntakeAgent, UserInput


intaker_module = importlib.import_module("nori.gen_agents.intaker")
...
def test_intaker_uses_llm_when_enabled(monkeypatch):
    ...
    monkeypatch.setattr(intaker_module.llms, "chat", fake_chat)
...
def test_intaker_text_uses_chat_json_helper(monkeypatch):
    ...
    monkeypatch.setattr(intaker_module.llms, "chat", fail_chat)
    monkeypatch.setattr(intaker_module.llms, "chat_json", fake_chat_json)
...
def test_intaker_falls_back_when_llm_fails(monkeypatch):
    ...
    monkeypatch.setattr(intaker_module.llms, "chat", broken_chat)
...
def test_intaker_falls_back_when_chat_json_fails(monkeypatch):
    def broken_chat_json(*args, **kwargs):
        raise intaker_module.llms.ChatJSONError("bad json", "not json")

    monkeypatch.setattr(intaker_module.llms, "chat_json", broken_chat_json)
    result = IntakeAgent(use_llm=True).run(
...
def test_intaker_vision_uses_chat_json_helper_and_isolates_failures(tmp_path, monkeypatch):
    ...
    def fake_chat_json(messages, *, usage="llm", _chat=None, **kwargs):
        calls.append({"messages": messages, "usage": usage, "_chat": _chat, "kwargs": kwargs})
        if len(calls) == 2:
            raise intaker_module.llms.ChatJSONError("bad vision json", "{")
        return {
...
def test_intaker_filters_unknown_role_and_normalizes_case(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(intaker_module.llms, "chat", lambda *a, **k: __import__("json").dumps(payload))
