...
const Canvas = ({ open, expanded, setExpanded, onClose, onTransform, onPublish, mode, setMode }) => {
    ...
    <aside style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12.5, fontWeight: 600, color: T.navy }}>小红书 · 深蓝幕布下的粉蝶兰</span>
          <span style={{
...
/* Main GenerationPage */
const GenerationPage = ({ initialPrompt, onBackHome }) => {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', width: '100%', background: T.surfaceWh, position: 'relative', overflow: 'hidden' }}>
      <Sidebar active="home" onNew={onBackHome} sessions={sessions} />
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative' }}>
        ...
        <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '24px 0 24px' }}>
          <div style={{
            ...
            <Bubble from="user">
              {initialPrompt || '我想生成一篇有关于猛男喜欢的粉色的植物科普与养护指导'}
            </Bubble>
            ...
            {/* Step 2 */}
            {stage >= 2 && <Step2HotPosts onSelectAngle={onSelectAngle} />}
            ...
            {/* Step 4 */}
            {stage >= 4 && <Step4Generate onAllDone={onAllDone} />}
...
const App = () => {
  const [page, setPage] = React.useState('home'); // 'home' | 'gen'
  ...
  return page === 'home'
    ? <HomePage onSubmit={goGen} />
    : <GenerationPage initialPrompt={prompt} onBackHome={() => setPage('home')} />;
};
