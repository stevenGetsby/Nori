"""Class-owned package contract for AccountPlannerAgent."""
...
class PreparedAccountPlannerInput:
    normalized: AccountPlannerInput


class AccountPlannerInputPreparer(AgentInputPreparer):
    """Restore AccountPlanner input evidence and search settings."""
    ...
    def prepare(
        ...
        links: list[str] | None,
    ) -> PreparedAccountPlannerInput:
        return PreparedAccountPlannerInput(normalized=self.normalize_input(user_input, images, links))
    ...
    def asset_context(self, path: str) -> dict[str, str]:
        suffix = Path(path).suffix.lower().lstrip(".")
        return {"path": path, "kind": suffix or "image"}


class AccountPlannerPromptBuilder(AgentPromptBuilder):
...
"""
    ...
    def __init__(self, input_preparer: AccountPlannerInputPreparer | None = None) -> None:
        self.input_preparer = input_preparer or AccountPlannerInputPreparer()

    def build_user_prompt(self, normalized: AccountPlannerInput, search_results: list[dict[str, Any]]) -> str:
        return self.user_prompt_template.format(
            text=normalized.text.strip() or "无",
            images=json_prompt([self.input_preparer.asset_context(path) for path in normalized.images]),
...
__all__ = [
    "AccountPlannerInputPreparer",
    "AccountPlannerPromptBuilder",
    "PreparedAccountPlannerInput",
]
