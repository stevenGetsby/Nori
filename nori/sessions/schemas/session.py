...
class SessionEvent:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any] | None) -> "SessionEvent":
        data = mapping(data)
        return cls(
            event_id=str(data.get("event_id") or new_id("event")),
...
class Session:
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> "Session":
        ...
        return cls(
            ...
            events=[SessionEvent.from_dict(item) for item in mapping_list(data.get("events"))],
            metadata=mapping(data.get("metadata")),
            created_at=str(data.get("created_at") or utc_now_iso()),
