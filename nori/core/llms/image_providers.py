...
from nori.core.contracts import ImageResultError


class ImageProviders:
    """Provider-specific image API adapters used by :class:`ImageClient`."""

    def collect_results(self, resp: Any) -> list[str]:
        ...
        return self.ensure_results(results)

    def ensure_results(self, results: list[str], model: Any | None = None) -> list[str]:
        if results:
            return results
    ...
    def relay_generate_with_references(
        self,
        bundle,
        prompt: str,
        ...
        *,
        ...
        for extra in payload_variants:
            ...
            try:
                ...
                return self.collect_results(resp)
            except Exception as exc:  # noqa: BLE001
                last_error = exc
                if not self._is_retryable_relay_reference_error(exc):
                    raise

        if last_error:
            if self._is_relay_base64_reference_error(last_error):
                raise RuntimeError(
        ...
        *,
        ...
        if ref_bytes_list:
            ...
            for raw in ref_bytes_list:
                parts.append(types.Part.from_bytes(data=raw, mime_type=sniff_mime(raw)))
            contents = parts
        else:
    ...
    @staticmethod
    def _is_relay_base64_reference_error(exc: Exception) -> bool:
        text = str(exc).lower()
